...
printf '[1/8] Closing only the review player; indexing is not interrupted...\n'
pkill -f "$APP_DIR/player.py" 2>/dev/null || true
sleep 1

printf '[2/8] Backing up current code and configuration...\n'
...
conn.executescript(
    ...
    CREATE INDEX IF NOT EXISTS idx_media_aliases_sha256
        ON media_aliases(sha256);
    """
)
conn.commit()
conn.close()
...
if "CREATE TABLE IF NOT EXISTS media_aliases" not in text:
    marker = """        CREATE INDEX IF NOT EXISTS idx_media_sha256 ON media(sha256);\n        CREATE TABLE IF NOT EXISTS transcript ("""
    replacement = """        CREATE INDEX IF NOT EXISTS idx_media_sha256 ON media(sha256);\n        CREATE TABLE IF NOT EXISTS media_aliases (\n            source_path TEXT PRIMARY KEY,\n            sha256 TEXT NOT NULL,\n            media_id INTEGER NOT NULL REFERENCES media(id) ON DELETE CASCADE,\n            canonical_path TEXT NOT NULL,\n            added_at TEXT NOT NULL\n        );\n        CREATE INDEX IF NOT EXISTS idx_media_aliases_sha256 ON media_aliases(sha256);\n        CREATE TABLE IF NOT EXISTS transcript ("""
    if marker not in text:
        raise SystemExit("Could not find the media schema marker.")
    text = text.replace(marker, replacement, 1)

discover_pattern = (
    r"def discover\(source_dir: Path\) -> list\[Path\]:\n"
    r".*?\n\ndef package_versions\(\) -> dict\[str, str\]:"
)
discover_replacement = '''def discover(source_dir: Path) -> list[Path]:
...
if count != 1:
    raise SystemExit(f"Could not replace discover(); count={count}")

if "Duplicate SHA-256 already cataloged" not in text:
    marker = """    digest = sha256_file(source)\n    probe = ffprobe(source)\n"""
    replacement = '''    digest = sha256_file(source)
...
'''
    if marker not in text:
        raise SystemExit("Could not find the digest/probe marker.")
    text = text.replace(marker, replacement, 1)

path.write_text(text, encoding="utf-8")
PY_PATCH_INDEXER

printf '[5/8] Installing the GUI with file intake, folder intake, status pills, and SHA-256 duplicate checks...\n'
...
if ! kill -0 "$PLAYER_PID" 2>/dev/null; then
    printf 'ERROR: Player exited during startup.\n' >&2
...
Added GUI controls:
  ...
  Add folder    Recursively add every supported media file
  Index now     Start indexing; or queue a rescan without interrupting active work

Duplicate handling:
  Exact paths are rejected as already added
  SHA-256 duplicate content is rejected even under a different filename
  The indexer also suppresses duplicate SHA-256 content during processing

Status indicators:
  COMPLETE      Green pill
  PROCESSING    Yellow pill
  QUEUED        Blue pill
  FAILED        Red pill

Registry:
  $DERIVED/intake-sources.json

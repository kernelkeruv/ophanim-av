...
secret_patterns = {
    ...
    "credential assignment": re.compile(
        r"(?i)\b(?:password|passwd|api[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret)\b"
        r"\s*[:=]\s*['\"][^'\"\n]{8,}['\"]"
    ),
}
personal_markers = [
...
for path in sorted(root.rglob("*")):
    ...
    if path.name in blocked_names or path.suffix.lower() in blocked_extensions:
        problems.append(f"blocked evidence or binary file: {relative_text}")
        continue

    size = path.stat().st_size
    if size > 2 * 1024 * 1024:
        problems.append(f"file exceeds 2 MiB allowlist limit: {relative_text} ({size} bytes)")
        continue
    ...
    for marker in personal_markers:
        if marker.casefold() in text.casefold():
            problems.append(f"machine-specific or personal marker '{marker}': {relative_text}")
